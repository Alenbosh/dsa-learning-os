...
export function LeetCodeSyncClient({ user }: { user: any }) {
    ...
    return (
        <div className="space-y-6">
            {/* LeetCode Sync Card */}
            <div className="bg-zinc-900/50 dark:bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden">
                {/* Header */}
                    ...
                    {!editing && savedUsername ? (
                        <div className="space-y-4">
                            <div className="flex items-center justify-between p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-lg">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs text-zinc-500 mb-0.5">Linked account</p>
                                        <p className="text-sm font-semibold text-emerald-400">@{savedUsername}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={unlockForEdit}
                                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-zinc-400 border border-zinc-700 rounded-lg hover:bg-zinc-800 hover:text-zinc-200 transition-all">
                                        <Pencil size={11} /> Edit
                                    </button>
                                    <button onClick={clearUsername}
                                        className="p-1.5 text-zinc-600 hover:text-rose-400 transition-colors" title="Remove">
                                        <X size={14} />
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between">
                                    <label className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Problems to import</label>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-semibold uppercase tracking-wider text-zinc-500">LeetCode Username</label>
                                <div className="flex gap-2">
                                    <input value={username}
                                        onChange={(e) => { setUsername(e.target.value); setVerified(null); setVerifyError("") }}
                                        onKeyDown={(e) => e.key === "Enter" && verifyUsername()}
                                        placeholder="e.g. neal_wu"
                                        className="flex-1 bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm placeholder-zinc-600 focus:outline-none focus:border-zinc-600 transition-colors"
                                    />
                                    <button onClick={verifyUsername} disabled={verifying || !username.trim()}
                                        className="flex items-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-sm font-medium rounded-lg transition-colors disabled:opacity-50">
