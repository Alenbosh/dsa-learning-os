...
export function ProblemModal({
    ...
    const [form, setForm] = useState({
        ...
        topicIds: problem?.topicLinks?.map((l: any) => l.topicId) ?? [],
    })
    ...
    useEffect(() => {
        fetch("/api/topics").then((r) => r.json()).then((data) => setTopics(data)).catch(() => { })
    }, [])
    ...
    function toggleTopic(id: string) {
        setForm((f) => ({
            ...f,
            topicIds: f.topicIds.includes(id) ? f.topicIds.filter((t) => t !== id) : [...f.topicIds, id],
        }))
    }
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2 space-y-1.5">
                            ...
                            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Two Sum" required className="field-input" />
                        </div>
                        <div className="col-span-2 space-y-1.5">
                            <label className="field-label">LeetCode URL</label>
                            <input type="url" value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} placeholder="https://leetcode.com/problems/..." className="field-input" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <select value={form.difficulty} onChange={(e) => setForm({ ...form, difficulty: e.target.value as any })} className="field-input" required>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="field-input" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="field-label">Time (min)</label>
                            <input type="number" min="1" value={form.timeTaken} onChange={(e) => setForm({ ...form, timeTaken: e.target.value })} placeholder="25" className="field-input" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <input type="number" min="1" value={form.attempts} onChange={(e) => setForm({ ...form, attempts: e.target.value })} className="field-input" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="field-label">Solved</label>
                            <select value={form.solved} onChange={(e) => setForm({ ...form, solved: e.target.value as any })} className="field-input">
                                <option value="">—</option>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select value={form.confidence} onChange={(e) => setForm({ ...form, confidence: e.target.value })} className="field-input">
                                <option value="">—</option>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input type="number" value={form.rating} onChange={(e) => setForm({ ...form, rating: e.target.value })} placeholder="1450" className="field-input" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <textarea value={form.pattern} onChange={(e) => setForm({ ...form, pattern: e.target.value })} placeholder="Use hashmap to store prefix frequency..." rows={3} className="field-input resize-none" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="field-label">Mistake Made</label>
                            <textarea value={form.mistake} onChange={(e) => setForm({ ...form, mistake: e.target.value })} placeholder="Forgot edge case when n=1..." rows={3} className="field-input resize-none" />
                        </div>
                    </div>
